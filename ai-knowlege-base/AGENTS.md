...
def fetch_trending(language: str, since: str = "daily") -> list[dict]:
    """从 GitHub Trending 获取指定语言的热门仓库。

    Args:
        language: 编程语言过滤器，如 "python"。
        since: 时间范围，可选 "daily"、"weekly"、"monthly"。

    Returns:
        仓库信息列表，每项包含 name、url、description、stars 等字段。

    Raises:
        FetchError: 当请求失败或响应解析出错时抛出。
    """
```
...
```jsonc
{
  ...
  // ── 状态 ──
  "status": "published",                // raw → analyzed → reviewed → published
...
```
raw ──(analyzer)──▶ analyzed ──(organizer)──▶ reviewed ──(publisher)──▶ published
```
...
2. **禁止裸 `print()`** — 使用 `logging` 或 `loguru`
3. **禁止 `except: pass` 或吞掉异常** — 至少记录日志
4. **禁止绕过 JSON schema 直接写入知识库** — 所有数据必须通过 Pydantic 模型校验
5. **禁止同步阻塞调用阻塞事件循环** — 外部 I/O 必须使用 `async/await` 或线程池
